...
BAn <- BA_MIC_biomass %>%
  ...
  #scale_color_manual(name = expression("BA MIC"[50]),values = c("#FFAA00", "#AA00AA" ))
# Add a black line for the median

FLCn <- 
  #scale_color_manual(name = expression("FLC MIC"[50]), values = c("#CC79A7",  "#009E73"))
#saving the FLC normalized figure


#BA_MIC_activity$MIC50 <- as.factor(BA_MIC_activity$MIC50)
...
# BAMBEC <- BA_MIC_MassAct %>% group_by(Strain) %>% summarise(MBEC = 100 - (Mass/PreMass)*100, Enviro = Enviro)
# FLCMBEC <- FLC_MIC_MassAct %>% group_by(Strain) %>% summarise(MBEC = 100 - ((Mass/Mass[Enviro == 0]*100)), Enviro = Enviro)
