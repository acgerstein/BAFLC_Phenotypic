...
# Load the data 
df <- read_csv(here("data_out", "DDA","210827_HSCAG.csv")) %>% filter(!Species == "C.tropicalis") 
...
# C. glb FLC, C. para BA,  and FLC RAD20 are not normally distributed 
...
# C. glb BA and FLC FoG20 are not normally distributed 
######################################################################
...
# Intraspecific range of FLC susceptibility and tolerance was broader in FLC 
######################################################################
...
# Kruskal-Wallis chi-squared = 63.425, df = 2, p-value = 1.688e-14

pairwise.wilcox.test(df_FLC$RAD20, df_FLC$Species,
                     p.adjust.method = "BH")
#               C.albicans C.glabrata
#   C.glabrata     6.2e-11    -         
#   C.parapsilosis 1.5e-05    8.8e-08     


kruskal.test(RAD20 ~ Species, data = df_BA)
# Kruskal-Wallis chi-squared = 39.795, df = 2, p-value = 2.284e-09

pairwise.wilcox.test(df_BA$RAD20, df_BA$Species,
                     p.adjust.method = "BH")
#              C.albicans C.glabrata 
#   C.glabrata     1.5e-08    -         
#   C.parapsilosis 0.0015     0.9844              
  


kruskal.test(FoG20 ~ Species, data = df_FLC)
# Kruskal-Wallis chi-squared = 82.036, df = 2, p-value < 2.2e-16
pairwise.wilcox.test(df_FLC$FoG20, df_FLC$Species,
                     p.adjust.method = "BH")
#               C.albicans C.glabrata 
#   C.glabrata     2.1e-11    -         
#   C.parapsilosis 2.1e-11    1.3e-05   

kruskal.test(FoG20 ~ Species, data = df_BA)
# Kruskal-Wallis chi-squared = 97.895, df = 2, p-value < 2.2e-16
pairwise.wilcox.test(df_BA$FoG20, df_BA$Species,
                     p.adjust.method = "BH")
#                C.albicans C.glabrata 
#   C.glabrata     < 2e-16    -         
#   C.parapsilosis 0.038      8.1e-07   
# 


# Calculate median RAD and FoG
...
df_mean <- df %>% group_by(Species, Drug) %>% 
  summarise(RAD20Med = mean(RAD20, na.rm = TRUE), 
            FoG20Med = mean(FoG20, na.rm = TRUE))
...
R_BA <- df %>% filter(Drug=="BA") %>% 
  ...
  annotate(y = RADFoG_MaxMin[RADFoG_MaxMin$Drug == "BA",]$RAD_min - 2, 
           geom = "text", x= c(1, 2, 3), label = c("b","a","a"),
           color="black", size = 6) +
  scale_x_discrete(label = c("C. albicans", 
                             "C. glabrata", "C. parapsilosis",
                             "C. tropicalis")) +
  ylab("Susceptibility (mm)") + expand_limits(y = c(-4,28))+
...
R_FLC <- df %>% filter(Drug=="FLC") %>% 
  ...
  annotate(y = RADFoG_MaxMin[RADFoG_MaxMin$Drug == "FLC",]$RAD_min - 2, 
           geom = "text", x= c(1, 2, 3), label = c("b","a","c"),
           color ="black", size = 6) +
...
T_FLC <- df %>% filter(Drug=="FLC") %>% 
  ...
  annotate(y = RADFoG_MaxMin[RADFoG_MaxMin$Drug == "FLC",]$FoG_max + 0.1, 
           geom = "text", x= c(1, 2, 3), label = c("a","b","c"),
           color ="black", size = 6) +
...
FLCBA_Cor <- df %>% filter(Drug == "BA") %>% rename("BA_RAD20" = "RAD20", "BA_FoG20" = "FoG20", "BA_RAD50" = "RAD50", "BA_FoG50" = "FoG50") %>% 
  ...
  select(!c(Drug)) %>% na.omit()
  
FLCBA_Cor %>% group_by(Species) %>% 
  summarise(cor = SpearmanCor(FLC_RAD20, FLC_FoG20)) 
# Species        cor$estimate $statistic $p.value $method                         $alternative
# <chr>                 <dbl>      <dbl>    <dbl> <chr>                           <chr>       
# C.albicans           -0.197    571000. 0.0190   Spearman's rank correlation rho two.sided   
# C.glabrata           -0.492     29249. 0.000327 Spearman's rank correlation rho two.sided   
# C.parapsilosis       -0.726      2295. 0.000291 Spearman's rank correlation rho two.sided  
FLCBA_Cor %>% group_by(Species) %>% 
  summarise(cor = SpearmanCor(BA_RAD20, BA_FoG20)) 
# Species        cor$estimate $statistic $p.value $method                         $alternative
# <chr>                 <dbl>      <dbl>    <dbl> <chr>                           <chr>       
# C.albicans           0.0682    444639.  0.420   Spearman's rank correlation rho two.sided   
# C.glabrata          -0.418      27802.  0.00277 Spearman's rank correlation rho two.sided   
# C.parapsilosis      -0.634       2173.  0.00269 Spearman's rank correlation rho two.sided

FLCBA_Cor %>% group_by(Species) %>% 
  summarise(cor = SpearmanCor(BA_RAD20, FLC_RAD20)) 
# Species        cor$estimate $statistic $p.value $method                         $alternative
# <chr>                 <dbl>      <dbl>    <dbl> <chr>                           <chr>       
# C.albicans         -0.0491     500605.    0.562 Spearman's rank correlation rho two.sided   
# C.glabrata          0.00261     19549.    0.986 Spearman's rank correlation rho two.sided   
# C.parapsilosis      0.267         975.    0.256 Spearman's rank correlation rho two.sided  
FLCBA_Cor %>% group_by(Species) %>% 
  summarise(cor = SpearmanCor(BA_FoG20, FLC_FoG20)) 
# Species        cor$estimate $statistic $p.value $method                         $alternative
# <chr>                 <dbl>      <dbl>    <dbl> <chr>                           <chr>       
# C.albicans         -0.0280     490557.    0.741 Spearman's rank correlation rho two.sided   
# C.glabrata          0.214       15401.    0.139 Spearman's rank correlation rho two.sided   
# C.parapsilosis     -0.00151      1332.    0.995 Spearman's rank correlation rho two.sided   
