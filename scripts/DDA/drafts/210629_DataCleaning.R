...
H_wide <- rbind(H1B1, H2B1) %>% 
 merge(rbind(H1B2, H2B2), by = c("Strain","Drug","Species", "Origin", "Year")) %>% 
  rename("RAD50_B01" = "RAD50.x", "RAD50_B02" = "RAD50.y",
         "FoG50_B01" = "FoG50.x", 
         "FoG50_B02" = "FoG50.y") %>% 
  select(!c(Biorep.x,Biorep.y)) %>% 
  merge(HB3, by = c("Strain","Drug","Species", "Origin", "Year")) %>% 
  rename("RAD50_B03" = "RAD50", "FoG50_B03" = "FoG50") %>% 
  select(!Biorep)

write_csv(H_wide, here("data_out", "DDA","210629_HSC_B1B2B3.csv"))
...
write_csv(AG_full, here("data_out", "DDA","210629_AG_B1B2.csv"))
...
write_csv(AGHSC, here("data_out", "DDA","210629_HSCAG.csv"))
