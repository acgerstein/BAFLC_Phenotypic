...
H1stat <- H1_ag %>% group_by(Species, Drug) %>% 
...
H2_df<-rbind(H2B1, H2B2) %>% mutate( Year="2018")

H2_ag<-rbind(H2B1, H2B2) %>% group_by(Strain, Drug, Species, Origin) %>% 
  summarise(RAD50=mean(RAD50), 
            FoG50=mean(FoG50), Type="HSC_2018")

H2stat<-H2_ag %>% group_by(Species, Drug) %>% 
  summarise(RAD50Med = median(RAD50, na.rm = TRUE),
            RAD50Mean = mean(RAD50, na.rm = TRUE),
            FoG50Med = median(FoG50, na.rm = TRUE),
            FoG50mean = mean(FoG50, na.rm = TRUE))
...
HSC<-rbind(H1_ag, H2_ag) %>% select(!Origin)
...
Sep1218 <- HSC %>% subset(Strain%in%Comb1218$Strain) %>% mutate(Type="Sep") %>%  merge(Info)
H_df<- rbind(H1_df, H2_df) %>% mutate(Type="Sep")



CombSep <- rbind(Comb1218,H_df) 
CS <- unite(CombSep, Year, c("Year","Type","Biorep"), sep = "_")
...
CombSep %>%  filter(Drug == "BA") %>% 
  ggplot() + geom_point(aes(x = Strain, y = RAD50 ))
