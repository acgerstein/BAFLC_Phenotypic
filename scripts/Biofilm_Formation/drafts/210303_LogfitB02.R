...
dfB01<-read_delim(here("210223_Evos_B01_editted.csv"), delim=",")%>%select(c("Filename","Background",
                                                                     "Fungi"))
dfB02<-read_delim(here("210225_Evos_B02.csv"), delim=",")%>%select(c("Filename","Background",
                                                                     "Fungi"))

#B02 has the same plate layout of B01. Can use the info file from B01
Info<- read_delim(here("Evos","210216_Evos_TL_B01","210218_Well_Info.csv"), delim=",")

#col<-c("A02"="#1C8678","A03"= "#ACDB1F","A04"= "#65CA44",
#       "A08" ="#28A26B","A10"= "#2B4279","A12"= "#371663",
#      "A17"= "#FEE51F","A18"= "#370755")

D<-df%>%separate(Filename,c("Well","Time"),"_t")
DF<-D%>%
...
DF %>% ggplot(aes(x=Time, y=Fungi, color=Concentration))+
